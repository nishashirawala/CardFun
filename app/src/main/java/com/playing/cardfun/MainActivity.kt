...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        player1card0.setOnClickListener{ player1CardClicked(player1card0, R.string.p1c0) }
        player1card1.setOnClickListener{ player1CardClicked(player1card1, R.string.p1c1) }
        player1card2.setOnClickListener{ player1CardClicked(player1card2, R.string.p1c2) }
        player1card3.setOnClickListener{ player1CardClicked(player1card3, R.string.p1c3) }
        player1card4.setOnClickListener{ player1CardClicked(player1card4, R.string.p1c4) }

        player2card0.setOnClickListener{ player2CardClicked(player2card0, R.string.p2c0) }
        player2card1.setOnClickListener{ player2CardClicked(player2card1, R.string.p2c1) }
        player2card2.setOnClickListener{ player2CardClicked(player2card2, R.string.p2c2) }
        player2card3.setOnClickListener{ player2CardClicked(player2card3, R.string.p2c3) }
        player2card4.setOnClickListener{ player2CardClicked(player2card4, R.string.p2c4) }

        show_btn.setOnClickListener{
            println(player1Cards.size)
            val t = Toast.makeText(applicationContext, " Points " + player1Cards.sumBy{ card -> card.points }, Toast.LENGTH_LONG)
            t.show()
        }
    }
    ...
    private fun player1CardClicked(playerCard: ImageView, tag: Int) {
        ...
        playerCard.setImageResource(newCard.imageId)
        player1Cards.remove(playerCard.getTag(tag))

        player1Cards.add(newCard)
        playerCard.setTag(tag, newCard)
    ...
    private fun distributeCards() {
        shuffle_btn.setOnClickListener{
            ...
            player1card0.setTag(R.string.p1c0, cards[0])
            player1card1.setImageResource(cards[1].imageId)
            player1card1.setTag(R.string.p1c1, cards[1])
            player1card2.setImageResource(cards[2].imageId)
            player1card2.setTag(R.string.p1c2, cards[3])
            player1card3.setImageResource(cards[3].imageId)
            player1card3.setTag(R.string.p1c3, cards[3])
            player1card4.setImageResource(cards[4].imageId)
            player1card4.setTag(R.string.p1c4, cards[4])

            player2card0.setImageResource(cards[5].imageId)
            player2card0.setTag(R.string.p2c0, cards[5])
            player2card1.setImageResource(cards[6].imageId)
            player2card1.setTag(R.string.p2c1, cards[6])
            player2card2.setImageResource(cards[7].imageId)
            player2card2.setTag(R.string.p2c2, cards[7])
            player2card3.setImageResource(cards[8].imageId)
            player2card3.setTag(R.string.p2c3, cards[8])
            player2card4.setImageResource(cards[9].imageId)
            player2card4.setTag(R.string.p2c3, cards[9])
